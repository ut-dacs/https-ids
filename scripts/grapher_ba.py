...
class CDF():
  ...
  def add_item(self, id, cur_host, cur_page):
    ...
    if self.data[id][cur_host]['count'] >= 5:
      ...
      if len(count) > 0:
        ...
        count = 0
      count += 1
      self.count.append(int(self.data[id][cur_host]['first']) - 1405408518)
  ...
  def process_files(self, ba_graph):
  ...
  def save_ba(self, ba_graph):

    count = defaultdict(int)
    for timecode in self.count:

      count[timecode] += 1
    for timecode in sorted(count):

      message = bytes("{0} {1}\n".format(timecode, count[timecode]), 'utf-8')
      ba_graph.write(message)
    
    #for i,timecode in enumerate(sorted(self.count.values())):
...
def main():
  ...
  with open('debug/ba_graph.txt', 'wb') as ba_graph:

    cdf.process_files(ba_graph)
    cdf.flush()
