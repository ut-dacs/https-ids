...
{2:.2f}% was data gathering, {3:.2f}% was data processing and {4:.2f}% was data filtering.".format(
  ...
  def __init__(self, logger):
    self.options = lib.config.read_config('ids')
  ...
  def process_files(self, nfdump_files):
    ...
    """
    worker = lib.worker.Worker()
    worker.logger = self.logger.getChild('worker')
    worker.nfdump_files = nfdump_files
    worker.signatures = self.signatures
    worker.run()
  ...
  def process_match(self, data):
    ...
    def worker(q, data):
      ...
      """
      thread = SigWorker()
      thread.logger = logging.getChild('sigworker')
      thread.data = data
